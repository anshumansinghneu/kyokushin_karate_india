...
export default function ProfilePage() {
    ...
    const handleDownloadCard = async () => {
        ...
        // Card size matching visual aspect ratio (1.6:1)
        const W = 160, H = 100;
        ...
        const accent = beltRank.startsWith('Black') || beltRank.includes('Dan')
            ? [220, 38, 38] as [number, number, number]
            : (BELT_COLORS[beltRank] || [229, 229, 229]);

        const ROLE_TITLES: Record<string, string> = { ADMIN: 'SHIHAN', INSTRUCTOR: 'SENSEI', STUDENT: 'KARATEKA' };
        ...
        // Valid thru - use membership status indicator
        const validThru = user?.membershipStatus === 'ACTIVE' ? 'Active' : user?.membershipStatus || '--';
        ...
        // QR
        const verifyUrl = `${window.location.origin}/verify/${user?.membershipNumber || ""}`;
        ...
        // Top accent line (belt color)
        doc.setFillColor(accent[0], accent[1], accent[2]);
        doc.rect(W * 0.1, 0, W * 0.8, 0.8, "F");

        // Corner radial glow effect (subtle)
        doc.setFillColor(accent[0], accent[1], accent[2]);
        ...
        // "FOUNDATION OF INDIA" with tricolor
        doc.setFontSize(5.5);
        doc.setFont("helvetica", "bold");
        ...
        doc.text(roleTitle, 13, roleY);

        // ── Member Name ──
        doc.setFont("helvetica", "bold");
        doc.setFontSize(18);
        doc.setTextColor(255, 255, 255);
        // Strip role title prefix from name if it's already there
        const cleanName = (user?.name || 'Member').replace(new RegExp(`^${roleTitle}\\s+`, 'i'), '').toUpperCase();
        const displayName = roleTitle !== 'KARATEKA' ? `${roleTitle} ${cleanName}` : cleanName;
        // Handle long names - reduce font size if needed
        const maxNameW = W - 16;
        let nameFontSize = 18;
        ...
        const gap = 2.5;
        const qrSize = 20;
        const availW = W - 16 - qrSize - 6; // padding - qr - gap
        const boxW = (availW - gap * 3) / 4;

        const stats = [
            { label: 'RANK', value: beltRank, dot: true },
        ...
        // ── Bottom accent line (belt color, subtle) ──
        doc.setFillColor(accent[0], accent[1], accent[2]);
