        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1.5 ml-[52px]">{events.length} event{events.length !== 1 ? 's' : ''} total</p>
                </div>
                <button onClick={() => handleOpenModal()} className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-red-600 to-rose-700 hover:from-red-500 hover:to-rose-600 text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-red-900/20">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-red-500/30 focus:bg-white/[0.05] transition-all"
                    />
                </div>
            ...
            {filteredEvents.length === 0 && !isLoading && (
                <div className="flex flex-col items-center justify-center py-20 border border-dashed border-white/[0.08] rounded-2xl">
                    <div className="w-16 h-16 rounded-2xl bg-red-500/[0.06] flex items-center justify-center mb-4">
                        <Calendar className="w-8 h-8 text-red-500/40" />
                    </div>
                    <p className="text-gray-400 font-medium mb-1">No events yet</p>
                    <p className="text-gray-600 text-sm mb-4">Create your first event to get started</p>
                    <button onClick={() => handleOpenModal()} className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-xl transition-colors">
                        <Plus className="w-4 h-4" /> Create Event
                    </button>
                </div>
            )}
                ...
                {filteredEvents.map((event) => {
                    ...
                    return (
                        <motion.div
                            ...
                            initial={{ opacity: 0, scale: 0.95 }}
                            animate={{ opacity: 1, scale: 1 }}
                            className="border border-white/[0.06] hover:border-white/[0.12] rounded-2xl overflow-hidden bg-white/[0.01] hover:bg-white/[0.025] transition-all group"
                        >
                            {/* Color accent bar */}
