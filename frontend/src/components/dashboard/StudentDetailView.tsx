...
import {
    ...
    ChevronRight, TrendingUp, Activity, Medal, FileText, Trash2
} from "lucide-react";
...
export default function StudentDetailView({ studentId, onClose }: StudentDetailViewProps) {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'belt-history' | 'training' | 'tournaments' | 'notes' | 'timeline'>('overview');
    ...
    const logProfileView = async () => {
        try {
            await api.post(`/users/${studentId}/profile-view`);
        } catch (error) {
            // Silently fail
            console.log('Profile view logging failed');
        }
    };
    ...
    const handleAddNote = async () => {
        ...
        try {
            await api.post(`/users/${studentId}/notes`, {
                noteText: newNote,
                isPrivate
            });
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {activeTab === 'notes' && user?.role !== 'STUDENT' && (
                            <motion.div
                                ...
                                exit={{ opacity: 0, x: -20 }}
                            >
                                <h3 className="text-lg font-bold text-white mb-6">Student Notes</h3>

                                {/* Add New Note */}
                                <div className="bg-white/5 border border-white/10 rounded-xl p-6 mb-6">
                                    <h4 className="text-white font-semibold mb-3">Add New Note</h4>
                                    <Textarea
                                        value={newNote}
                                        onChange={(e) => setNewNote(e.target.value)}
                                        placeholder="Enter your note about this student..."
                                        className="mb-3 bg-white/5 border-white/10 text-white min-h-[100px]"
                                    />
                                    ...
                                    <div className="space-y-4">
                                        {notes.map((note) => (
                                            ...
                                            >
                                                <div className="flex items-start justify-between mb-3">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-red-600 to-orange-500 flex items-center justify-center">
                                                            {note.author.profilePhotoUrl ? (
                                                                <img
                                                                    src={getImageUrl(note.author.profilePhotoUrl)!}
                                                        ...
                                                        <div>
                                                            <div className="flex items-center gap-2">
                                                                <h4 className="text-white font-semibold">{note.author.name}</h4>
                                                                {note.isPrivate && (
                                                            ...
                                                            <p className="text-xs text-gray-500">
                                                                {note.author.role} • {formatDate(note.createdAt)}
                                                            </p>
                                                        </div>
                                                    </div>
                                                ...
                                                <p className="text-gray-300 text-sm whitespace-pre-wrap pl-13">
                                                    {note.noteText}
                                                </p>
                                            </div>
                                        ))}
